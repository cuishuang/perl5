...
sub read_commit_log {
    ...
    while (defined(my $line= <$fh>)) {
        ...
        my $author_name_mm= _make_name_author_info(
            $author_info, $commit_info, "author_name_mm");

        my $committer_name_mm= _make_name_author_info(
            $author_info, $commit_info, "committer_name_mm");

        my $author_name_real= _make_name_simple($commit_info, "author");

        my $committer_name_real= _make_name_simple($commit_info, "committer");

        _check_name_mailmap($mailmap_info, $author_name_mm,
            $author_name_real, $commit_info, "author name");
...
sub read_authors {
    ...
    while (defined(my $line=<$in_fh>)) {
        ...
        $author_info{"email2name"}{lc($email)}= $name
            if $email and $name and $email ne "unknown";
...
sub update_mailmap {
    ...
    foreach my $line (
        @$mailmap_preamble,
        _sorted_hash_keys($mailmap_hash),
    ) {
...
sub _check_name_mailmap {
    ...
    if (!$mailmap_info->{$P2O}{$name}) {
        warn encode_utf8 sprintf "Unknown %s '%s' in commit %s '%s'\n%s", $descr,
            $name, $commit_info->{"abbrev_hash"}, $commit_info->{"commit_subject"},
            $blurb;
